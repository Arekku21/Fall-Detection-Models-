...
def load_custom_model(model_type):
    ...
    if model_type =="1 Step":
        ...
        model = torch.hub.load('ultralytics/yolov5', 'custom', path='D:\Documents\GitHub\Yolo-Interface-using-Streamlit\models\\lowlight.pt', force_reload=False)
        model.conf = 0.4
...
def main():
    global model_choice,model_chosen,tf_model,frame_counter
    ...
    #     if alert_call == "falling":
    #         st.info("ALERT PERSON IS FALLING")
