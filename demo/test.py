...
# image = cv2.imread('lowlight.jpeg')
...
# gamma = 1.5
# image_gamma = np.power(image_gray, gamma)

# # Apply histogram equalization to improve contrast
# image_equalized = cv2.equalizeHist((image_gamma * 255).astype(np.uint8))

# # Convert the images back to uint8 format
# image_gamma_uint8 = (image_gamma * 255).astype(np.uint8)
# image_equalized_uint8 = image_equalized.astype(np.uint8)

# # Display the original and augmented images
# cv2.imshow('Original Image', image)
# cv2.imshow('Gamma Correction', image_gamma_uint8)
# cv2.imshow('Histogram Equalization', image_equalized_uint8)
...
infrared_image = cv2.imread('lowlight.jpeg', cv2.IMREAD_GRAYSCALE)

# Apply false color mapping
visible_image = cv2.applyColorMap(infrared_image, cv2.COLORMAP_HOT)

# Display the infrared and visible images
